...
public class Test16 {
	public static void main(String[] args) {
		...
		outer_while://이중 반복문을 중단하기 위한 임의의 레이블명 지정
			while(true) {//무한루프문
				System.out.println("\n1- 비빔밥");
				...
				for(;;) {//for무한루프문
					System.out.print("\n음식 계산할 값을 입력(계산 종료는 0,전체 종료는 99)>>");
					num = Integer.parseInt(scan.nextLine());//입력받은 문자를 정수숫자로 변경
					...
					if(num == 99) {
						break outer_while;//전체 종료 => for반복문,while반복문 모두 종료
					}
					
				   switch(menu) {
				   	case 1:
				   		System.out.println("비빔밥값="+(num*1));
				   		break;
				   	case 2:
				   		System.out.println("고등어 자반값="+(num*2));
				   		break;
				   	case 3:
				   		System.out.println("오징어 볶음값="+(num*3));
				   		break;
				   }//switch
				}//for
			}//while
